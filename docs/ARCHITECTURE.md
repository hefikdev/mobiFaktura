...
│                                                                  │
│  ┌─────────────┐  ┌──────────────┐  ┌────────────────┐        │
│  │ Middleware  │→ │  tRPC Router │→ │ Business Logic │        │
│  │ (Auth/IP)   │  │              │  │                │        │
│  └─────────────┘  └──────────────┘  └────────────────┘        │
│                                                                  │
│  ┌──────────────────────────────────────────────────────┐      │
...
│                                                                  │
│  Volumes:                                                        │
│  • /app/logs → app_logs (persistent logs)                      │
│                                                                  │
│  Environment:                                                    │
│  • NODE_ENV=production                                          │
...
```
┌──────┐         ┌────────────┐         ┌──────────┐         ┌──────────┐
│Client│         │ Middleware │         │   tRPC   │         │ Database │
└──┬───┘         └─────┬──────┘         └────┬─────┘         └────┬─────┘
   │                   │                     │                    │
   │  POST /login      │                     │                    │
   ├──────────────────>│                     │                    │
...
│   │   │   ├── admin/          # Admin panel
│   │   │   └── analytics/       # Analytics (accountant)
│   │   │
│   │   ├── api/                 # API Routes
...
│   │           └── analytics.ts
│   │
│   ├── middleware.ts            # Next.js middleware (auth + IP)
...
│     • Output: Console + Files (/app/logs/)                     │
│     • Files: app.log (all), error.log (errors)                 │
│     • Rotation: Manual or automated                             │
...
```
┌─────────────────────────────────────────────────────────────────┐
│                     Scheduled Tasks (Daily 1 AM)                 │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  1. cleanOldLoginLogs()                                         │
│     • Delete login logs older than 30 days                      │
│     • Keeps audit trail manageable                              │
│                                                                  │
│  2. cleanExpiredSessions()                                      │
│     • Remove expired session records                            │
│     • Prevents table bloat                                      │
│                                                                  │
│  3. cleanOldLoginAttempts()                                     │
...
```
┌─────────────────────────────────────────────────────────────────┐
│                   Docker Network: mobifaktura_network           │
│                          (Bridge Driver)                         │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  Internal Communication (No External Access):                    │
│                                                                  │
│  ┌─────────────┐                                                │
│  │     app     │                                                │
│  │ (Next.js)   │                                                │
...
│  • API responses via Next.js cache                              │
│                                                                  │
│  Build:                                                          │
